...
<template>
  ...
  >
    ...
    >
      ...
      <div v-if="!desktop" class="menu-bar" data-tauri-drag-region>
        <div
          ...
          class="menu-group"
        >
          <button @click="menu = menu === category ? '' : category">
            {{ category }}
          </button>
          <div v-if="menu === category" class="menu-popover">
            <button
            ...
            >
              {{ command.title
              }}<small>{{ command.shortcut?.replaceAll("Mod", "⌘") }}</small>
            </button>
          </div>
        </div>
      </div>
    ...
    <div class="workspace" @click="menu = ''">
      <FileSidebar
        v-if="ui.sidebar"
        @open="
      ...
      >
        <div class="document-view-menu">
          <button
            ...
            title="文档视图与导出"
            aria-label="文档菜单"
            @click="documentMenu = !documentMenu"
          >
            <UiIcon name="more" />
          </button>
          <div
            v-if="documentMenu"
            class="menu-popover"
            @click="documentMenu = false"
          >
            <button @click="controller.execute('view.wide')">
              {{ settings.value.wideEditor ? "标准宽度" : "宽屏模式" }}
            </button>
            <button @click="ui.reading = !ui.reading">
              {{ ui.reading ? "编辑模式" : "阅读模式" }}
            </button>
            <button @click="presentation">演示模式</button>
            <button @click="controller.execute('file.print')">导出 PDF…</button>
            <button @click="controller.execute('file.word')">导出 Word…</button>
            <button :disabled="!doc.path" @click="copyPath">
              复制文件路径
            </button>
          </div>
        </div>
