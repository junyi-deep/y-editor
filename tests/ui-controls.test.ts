...
vi.mock("../src/editor/MarkdownEditor.vue", () => ({ default: { template: '<textarea aria-label="测试正文" />' } }));
...
it("opens the document actions menu, then dismisses it on Escape", async () => {
  ...
  expect(document.querySelector('[role="menuitem"][data-disabled]')?.textContent).toContain("复制文件路径");
...
it("keeps native select/datalist and direct engine imports out of business surfaces", () => {
  const scan = (dir: string): string[] => readdirSync(dir, { withFileTypes: true }).flatMap(entry => entry.isDirectory() ? scan(join(dir, entry.name)) : [join(dir, entry.name)]);
  for (const file of scan("src").filter(file => /\.(vue|ts)$/.test(file))) {
    ...
    if (!file.includes("components/ui/")) expect(source, file).not.toMatch(/from ["']reka-ui["']/);
  }
});
